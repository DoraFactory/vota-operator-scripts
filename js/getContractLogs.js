...
const provider = 'https://vota-testnet-api.dorafactory.org/'(
	// const provider = 'http://127.0.0.1:8000';
	...
	async () => {
		...
		function handleMessage(event) {
			...
			const pubkey = event.encPubKey
				.split(',')
				.map(n => BigInt(n.slice(1, n.length - 1)));
			const msg = event.message
				.split('[')[1]
				.split(']')[0]
				.split(', ')
				.map(n => BigInt(n.slice(8, n.length - 1)));
    ...
    publishMessageEvents(orderBy: [MSG_CHAIN_LENGTH_ASC],
      ...
      totalCount
		...
		fs.writeFileSync(
			...
			JSON.stringify(stringizing({ messages, states }), undefined, 2)
		);
	}
)();
