		...
		try {
			...
			if (period.status === 'pending') {
				if (Number(end_time) < now) {
					...
					const { msgCount, tallyCount } =
						countMsgAndTally(commitments);
					...
					for (let i = 0; i < msgCount; i += 1) {
						...
						const newStateCommitment =
							commitments[`msg_${tailNum}`];
						...
						try {
							const process_message_res =
								await maci.processMessage({
									newStateCommitment,
									proof,
								});
					...
					const stop_processing_res =
						await maci.stopProcessingPeriod();
